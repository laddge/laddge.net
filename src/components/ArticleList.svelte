...
<ul>
  {#each articles as article}
    <li class="py-4 flex flex-col sm:flex-row items-center gap-3 break-words">
      <a
        href={article.href}
        target={article.articleType ? '_blank' : ''}
        class="btn btn-ghost w-full h-auto border-none p-0 flex-none sm:w-32 sm:max-w-[30vw] aspect-[1200/630] rounded-xl bg-base-200 bg-no-repeat bg-center bg-[length:2rem_2rem] overflow-hidden"
      ...
      <div class="min-w-0 grow w-full">
        <a
          href={article.href}
          target={article.articleType ? '_blank' : ''}
          class="flex max-w-fit btn btn-sm btn-ghost normal-case text-xl sm:text-2xl text-left h-auto"
        >
          <span class="min-w-0"
        ...
        <div class="mt-0.5 px-3 flex items-center gap-1">
          ...
          {#if article.articleType == 1}
            <div class="px-1 flex items-center gap-1">
              <div class="w-2 h-2 flex-none rounded-full bg-[#55c500]" />
              Qiita
            </div>
          {:else if article.articleType == 2}
            <div class="px-1 flex items-center gap-1">
              <div class="w-2 h-2 flex-none rounded-full bg-[#3ea8ff]" />
              Zenn - Article
            </div>
